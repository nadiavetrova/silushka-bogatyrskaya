  ...
  return (
    <div className="flex flex-col" style={{ minHeight: "calc(100vh - 140px)" }}>
      ...
      <div className="flex gap-1 mb-4 bg-[#1a1208]/60 rounded-xl p-1 border border-[#3a3530]/40">
        <button
          onClick={() => setTab("chat")}
          className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all ${tab === "chat" ? "bg-[#8b2525] text-[#e8dcc8]" : "text-[#9b7a4a] hover:text-[#d4bc8e]"}`}
        >
          💬 Чат с Берегиней
        </button>
        <button
          onClick={() => setTab("plans")}
          className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all ${tab === "plans" ? "bg-[#8b2525] text-[#e8dcc8]" : "text-[#9b7a4a] hover:text-[#d4bc8e]"}`}
        >
